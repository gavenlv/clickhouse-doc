...
-- 
-- 本文件涵盖:
--   1. INTERVAL 语法 - 时间间隔加减
--   2. add/subtract 函数 - addDays(), subtractMonths()
--   3. Unix 时间戳运算 - 秒级精度运算
--   4. dateDiff() - 计算时间差
--   5. 滚动窗口计算 - 累计和滑动窗口
--   6. 业务场景应用 - 订阅到期、年龄计算
...
--   
--   运算方式:
--   1. INTERVAL: now() + INTERVAL 1 DAY
--   2. 函数:     addDays(now(), 1)
--   3. Unix:     toDateTime(toUnixTimestamp(now()) + 86400)
-- 
-- ================================================================================
-- INTERVAL 运算工作原理
-- ================================================================================
...
--   └─────────────────────────────────────────────────────────────────────────┘
--   
--   原始时间: 2024-01-20 12:00:00
--   
--   + INTERVAL 1 SECOND   → 2024-01-20 12:00:01    (秒)
--   + INTERVAL 1 MINUTE   → 2024-01-20 12:01:00    (分钟)
--   + INTERVAL 1 HOUR     → 2024-01-20 13:00:00    (小时)
--   + INTERVAL 1 DAY      → 2024-01-21 12:00:00    (天)
--   + INTERVAL 1 WEEK     → 2024-01-27 12:00:00    (周)
--   + INTERVAL 1 MONTH    → 2024-02-20 12:00:00    (月 - 智能处理)
--   + INTERVAL 1 QUARTER  → 2024-04-20 12:00:00    (季度)
--   + INTERVAL 1 YEAR     → 2025-01-20 12:00:00    (年)
--   
--   特殊情况 (月末智能处理):
--   2024-01-31 + INTERVAL 1 MONTH → 2024-02-29 (闰年)
--   2023-01-31 + INTERVAL 1 MONTH → 2023-02-28 (平年)
-- 
-- ================================================================================
-- dateDiff() 计算原理
-- ================================================================================
...
--   00:00:00                           14:30:45
--   
--   dateDiff('second', start, end)  →  1681845  秒
--   dateDiff('minute', start, end)  →  28030    分钟
--   dateDiff('hour',   start, end)  →  467      小时
--   dateDiff('day',    start, end)  →  19       天
--   dateDiff('week',   start, end)  →  2        周
--   dateDiff('month',  start, end)  →  0        月
--   dateDiff('year',   start, end)  →  0        年
--   
--   注意: dateDiff 返回的是完整单位数，会向下取整!
-- 
-- ================================================================================
-- 滚动窗口计算原理
-- ================================================================================
...
--   └─────────────────────────────────────────────────────────────────────────┘
--   
--   数据点:  v1   v2   v3   v4   v5   v6   v7   v8   v9   v10
--           ─┬────┬────┬────┬────┬────┬────┬────┬────┬────┬──→ 时间
--            │    │    │    │    │    │    │    │    │    │
--            │    │    │    │    │    │    │    │    │    │
--   3点窗口:  └────┴────┴────┘    │    │    │    │    │    │    avg(v1,v2,v3)
...
--   
--   SQL 实现:
--   avg(value) OVER (ORDER BY event_time
