...
-- 本文件涵盖:
--   1. 并行插入配置 - 多线程并行写入优化
--   2. 批量大小优化 - 最小批量行数/字节数设置
--   3. 异步插入配置 - 高频小批量导入场景
--   4. 外部数据源导入 - GCS/Parquet格式优化
--   5. Native格式导入 - 最快导入方式
--   6. 分布式表导入 - 集群并行写入策略
--   7. 导入性能监控 - 实时监控导入进度
-- 
...
-- │  ┌─────────┐      ┌─────────────────────┐      ┌─────────┐                │
-- │  │ 数据文件 │─────►│ 多线程并行 + 批量    │─────►│ 目标表  │                │
-- │  │ Native  │      │ max_threads=8       │      │         │                │
-- │  └─────────┘      └─────────────────────┘      └─────────┘                │
-- │                    ~2分钟                                                    │
-- │                                                                             │
-- │  性能提升: 5x (通过格式+并行+批量优化)                                         │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ┌─────────────────────────────────────────────────────────────────────────────┐
...
-- │  参数                        默认值      推荐值       说明                   │
-- │  ────────────────────────────────────────────────────────────────────────  │
-- │  max_insert_threads          1          8            并行写入线程数          │
-- │  min_insert_block_size_rows  1048576    1000000      最小批量行数            │
-- │  min_insert_block_size_bytes  268435456  104857600    最小批量字节数(100MB)   │
-- │  async_insert                0          1            异步插入开关            │
-- │  async_insert_max_data_size  10485760   104857600    异步最大数据量(100MB)   │
-- │  async_insert_busy_timeout_ms 100000    10000        异步超时(10秒)         │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
