...
-- 
-- 本文件涵盖:
--   1. 物化视图预聚合 - 常用维度组合的预计算
--   2. 跳数索引优化 - 减少数据扫描量
--   3. Projection多维索引 - 自动选择最优存储路径
--   4. 查询缓存配置 - 缓存重复查询结果
--   5. PREWHERE优化 - 过滤前置减少IO
--   6. 性能监控分析 - 慢查询诊断与优化
--   7. TTL生命周期管理 - 数据自动清理策略
-- 
-- 目标: 查询响应 < 10秒
...
-- ├─────────────────────────────────────────────────────────────────────────────┤
-- │                                                                             │
-- │  优化策略          适用场景           加速效果      实施复杂度                  │
-- │  ────────────────────────────────────────────────────────────────────────  │
-- │  分区裁剪          时间/批次范围查询    ★★★☆☆       低 (设计时考虑)            │
-- │  PREWHERE过滤      高选择性过滤条件    ★★★★☆       低 (改写查询)              │
-- │  跳数索引          高频过滤字段        ★★★★☆       中 (ALTER TABLE)           │
-- │  物化视图          固定聚合模式        ★★★★★       中 (创建MV+目标表)          │
-- │  Projection        多种查询模式        ★★★★☆       中 (ALTER TABLE)           │
-- │  查询缓存          重复查询            ★★★★★       低 (SET参数)               │
-- │                                                                             │
-- │  推荐组合: 分区裁剪 + PREWHERE + 物化视图 + 查询缓存 = 最优性能                  │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ┌─────────────────────────────────────────────────────────────────────────────┐
...
-- │  │                                                                 │        │
-- │  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │        │
-- │  │  │ mv_agg_      │  │ mv_metric_   │  │ mv_yearly_   │         │        │
-- │  │  │ region_prod  │  │ hierarchy    │  │ agg          │         │        │
-- │  │  │ (区域×产品)   │  │ (指标层级)    │  │ (年度汇总)    │         │        │
-- │  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘         │        │
-- │  └─────────┼────────────────┼────────────────┼───────────────────┘        │
-- │            │                │                │                            │
-- │            ▼                ▼                ▼                            │
-- │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                    │
-- │  │ agg_region_  │  │ agg_metric_  │  │ agg_yearly   │                    │
-- │  │ product_     │  │ hierarchy    │  │              │                    │
-- │  │ customer     │  │              │  │              │                    │
-- │  │ (目标表)      │  │ (目标表)      │  │ (目标表)      │                    │
-- │  └──────────────┘  └──────────────┘  └──────────────┘                    │
...
-- │                    ~10ms                                                   │
-- │                                                                             │
-- │  缓存配置: use_query_cache=1, cache_size=1GB, ttl=300s                      │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
