...
-- 
-- 本文件涵盖:
--   1. 指标层级数据生成 - 4层父子结构指标树
--   2. 年月映射数据生成 - 60个月份的时间维度
--   3. 高基数维度数据生成 - 90个维度列的测试数据
--   4. 二维数组预测数据生成 - 20指标×60月份的预测值
--   5. 数据质量标记生成 - 模拟真实数据质量场景
-- 
...
-- │       ▼  ▼                    ▼                    ▼                  │
-- │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐              │
-- │  │产品收入(2)│  │服务收入(3)│  │其他收入(4)│  │增长率(20) │              │
-- │  └────┬─────┘  └────┬─────┘  └────┬─────┘  └──────────┘              │
-- │       │             │             │                                   │
-- │       ▼             ▼             ▼                                   │
-- │  ┌──────────┐  ┌──────────┐  ┌──────────┐                            │
-- │  │线上收入(5)│  │咨询收入(7)│  │许可收入(9)│                            │
-- │  │线下收入(6)│  │支持收入(8)│  │订阅收入(10)│                            │
-- │  └────┬─────┘  └──────────┘  └──────────┘                            │
-- │       │                                                                  │
-- │       ▼                                                                  │
-- │  ┌──────────┐  ┌──────────┐  ┌──────────┐                              │
-- │  │移动端(11) │  │网页端(12) │  │APP(13)   │                              │
-- │  └──────────┘  └──────────┘  └──────────┘                              │
-- │                                                                             │
-- │  特点: 父指标值 = 子指标值之和，支持层级聚合和下钻                               │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ┌─────────────────────────────────────────────────────────────────────────────┐
...
-- │                                                                             │
-- │  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐  │
-- │  │ 1. 元数据    │───►│ 2. 时间映射  │───►│ 3. 维度数据  │───►│ 4. 预测数据  │  │
-- │  │ (20行)      │    │ (60行)      │    │ (1000行)    │    │ (1000行)    │  │
-- │  └─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘  │
-- │        │                  │                  │                  │          │
-- │        ▼                  ▼                  ▼                  ▼          │
-- │  metric_metadata    month_mapping    trans_dimensions   prediction_values  │
-- │                                                                             │
-- │  数据规模: 1000行 × 20指标 × 60月份 = 1,200,000 等效行                        │
-- │  实际存储: 1000行 (压缩率 99.92%)                                             │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
