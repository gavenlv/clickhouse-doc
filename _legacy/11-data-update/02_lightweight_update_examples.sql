...
--   │  ┌──────────────────────────────────────────┐                         │
--   │  │ id=1, status='pending'                   │                         │
--   │  │ id=2, status='pending'                   │  读取 + 写入             │
--   │  │ id=3, status='active'                    │  → 重写整个 Part         │
--   │  └──────────────────────────────────────────┘                         │
--   │                    │                                                   │
--   │                    ▼                                                   │
--   │  Part 1 (新数据)                                                       │
--   │  ┌──────────────────────────────────────────┐                         │
--   │  │ id=1, status='active'  ← 已修改          │                         │
--   │  │ id=2, status='active'  ← 已修改          │                         │
--   │  │ id=3, status='active'                    │                         │
--   │  └──────────────────────────────────────────┘                         │
--   │  磁盘 I/O: 读取整个 Part + 写入整个 Part                               │
--   └────────────────────────────────────────────────────────────────────────┘
--   
--   轻量级更新:
--   ┌────────────────────────────────────────────────────────────────────────┐
...
--   │  ┌──────────────────────────────────────────┐                         │
--   │  │ id=1, status='pending'                   │                         │
--   │  │ id=2, status='pending'                   │  不修改原数据           │
--   │  │ id=3, status='active'                    │                         │
--   │  └──────────────────────────────────────────┘                         │
--   │                    +                                                   │
--   │  增量文件 (只存储变更)                                                  │
--   │  ┌──────────────────────────────────────────┐                         │
--   │  │ id=1, status='active'                    │  只写入变更行           │
--   │  │ id=2, status='active'                    │  磁盘 I/O 极小          │
--   │  └──────────────────────────────────────────┘                         │
--   │                    ↓                                                   │
--   │  查询时自动合并: 读取原数据 + 应用增量变更                              │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
-- 轻量级更新执行流程
...
--   适用场景:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  Mutation: 大批量更新 (> 30% 数据), 一次性数据修正                      │
--   │  轻量级更新: 高频小批量更新 (< 10% 数据), 实时状态更新                  │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
-- 使用限制
...
--   └─────────────────────────────────────────────────────────────────────────┘
--   
--   1. 版本要求: ClickHouse 23.8+
--   2. 引擎支持: MergeTree 系列引擎
--   3. 主键要求: WHERE 条件必须能利用主键索引
--   4. 存储开销: 增量文件占用额外空间, 直到合并
--   5. 查询开销: 查询时需要合并增量, 略有性能损失
--   
--   不适用场景:
--   - 更新条件不能利用主键
--   - 更新超过 30% 的数据
