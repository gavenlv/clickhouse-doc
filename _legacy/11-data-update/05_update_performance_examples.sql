...
--   性能公式:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  更新时间 ∝ (数据量 × 分区数) / (选择性 × 并行度)                       │
--   │                                                                         │
--   │  优化目标: 减少数据量, 增加选择性, 提高并行度                            │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
-- 索引优化原理
...
--   无索引更新:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  UPDATE status = 'active' WHERE email LIKE '%@example.com'             │
--   │                                                                         │
--   │  扫描所有数据 → 匹配条件 → 重写匹配的数据                               │
--   │  数据量: 100% 表数据                                                    │
--   └────────────────────────────────────────────────────────────────────────┘
...
--   有索引更新:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  UPDATE status = 'active' WHERE user_id IN (1, 2, 3)                   │
--   │                                                          ↑              │
--   │                                              user_id 是主键             │
--   │  利用主键索引 → 只扫描相关 Part → 重写数据                              │
--   │  数据量: < 1% 表数据                                                    │
--   └────────────────────────────────────────────────────────────────────────┘
--   
--   性能对比:
--   - 无索引: 100万行更新 → 30秒
--   - 有索引: 100万行更新 → 3秒 (10倍提升)
-- 
-- ================================================================================
-- 分区优化原理
-- ================================================================================
...
--   │    AND toYYYYMM(created_at) = '202401'  ← 分区条件                     │
--   │                                                                         │
--   │  只扫描 202401 分区 → 匹配条件 → 重写                                   │
--   │  分区扫描: 1个分区                                                      │
--   └────────────────────────────────────────────────────────────────────────┘
--   
--   性能对比:
...
--   一次性更新:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  UPDATE status = 'active' WHERE status = 'pending'                     │
--   │                                                                         │
--   │  影响: 1000万行, 10GB 数据                                              │
--   │  磁盘I/O: 20GB (读 + 写)                                                │
--   │  执行时间: 5分钟                                                        │
...
--   分批更新:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  批次 1: WHERE user_id BETWEEN 1 AND 100000                            │
--   │  批次 2: WHERE user_id BETWEEN 100001 AND 200000                       │
--   │  批次 3: WHERE user_id BETWEEN 200001 AND 300000                       │
--   │  ...                                                                    │
--   │                                                                         │
--   │  每批影响: 10万行, 100MB 数据                                           │
--   │  单批执行: 3秒                                                          │
...
--   └─────────────────────────────────────────────────────────────────────────┘
--   
--   1. Mutation 进度: system.mutations
--   2. CPU/内存使用: system.processes
--   3. 磁盘 I/O: system.asynchronous_metrics
--   4. 后台队列: system.replication_queue
--   
--   关键指标:
--   - progress: 更新进度 (0-1)
--   - parts_to_do: 待处理 Part 数量
--   - cpu_time_nanoseconds: CPU 时间
--   - memory_usage: 内存使用
--   - read_bytes/written_bytes: 磁盘读写量
-- 
-- ================================================================================
