...
--   2. 许可性策略 - PERMISSIVE
--   3. 多策略组合 - AND/OR 逻辑
--   4. 用户级别过滤 - current_user()
--   5. 部门级别过滤 - 用户设置
--   6. 时间范围过滤 - 时间窗口
--   7. 字典驱动策略 - 动态权限
--   8. 多租户隔离 - tenant_id
--   9. 数据敏感级别 - sensitivity_level
--   10. 地理区域过滤 - region
-- 
-- 行级安全原理:
-- 
...
--   应用行策略后:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │ SELECT * FROM orders WHERE department = current_user_settings['dept']; │
--   │                         ↑                                              │
--   │                    自动添加的过滤条件                                   │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
...
--   └─────────────────────────────────────────────────────────────────────────┘
--   
--   RESTRICTIVE (限制性) - AND 逻辑:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │ 策略1: user_id = current_user()                                        │
--   │ 策略2: status = 'active'                                               │
--   │                                                                        │
--   │ 结果: user_id = current_user() AND status = 'active'                  │
--   │ 两个条件都必须满足                                                       │
--   └────────────────────────────────────────────────────────────────────────┘
--   
--   PERMISSIVE (许可性) - OR 逻辑:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │ 策略1: user_id = current_user()                                        │
--   │ 策略2: is_public = 1                                                   │
--   │                                                                        │
--   │ 结果: user_id = current_user() OR is_public = 1                        │
--   │ 满足任意一个条件即可                                                     │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
-- 多租户隔离架构:
...
--   应用服务器
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  用户请求 → 提取 tenant_id → 连接 ClickHouse                           │
--   └────────────────────────────────────────────────────────────────────────┘
--                         │
--                         ▼
--   ClickHouse 行策略:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │ CREATE ROW POLICY tenant_filter                                        │
--   │ ON orders USING tenant_id = current_user_settings['tenant_id']         │
--   │ AS RESTRICTIVE TO tenant_users;                                        │
--   └────────────────────────────────────────────────────────────────────────┘
--                         │
--                         ▼
--   数据表:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │ tenant1 的用户 → 只看到 tenant_id = 'tenant1' 的数据                   │
--   │ tenant2 的用户 → 只看到 tenant_id = 'tenant2' 的数据                   │
--   │ tenant3 的用户 → 只看到 tenant_id = 'tenant3' 的数据                   │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
