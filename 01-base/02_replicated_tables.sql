...
-- │                                                              │
-- │   复制流程:                                                  │
-- │   1. Primary 写入本地 + 记录 ZooKeeper                      │
-- │   2. Backup 监听 ZooKeeper 发现新数据                       │
-- │   3. Backup 从 Primary 拉取数据                              │
-- │   4. 两副本数据同步                                          │
-- │                                                              │
-- └─────────────────────────────────────────────────────────────┘
...
-- │                                                              │
-- │  配置位置: /etc/clickhouse-server/config.d/macros.xml        │
-- │                                                              │
-- └─────────────────────────────────────────────────────────────┘
...
-- │  路径配置项:                                                 │
-- │                                                              │
-- │  - default_replica_path: ZooKeeper 中副本路径模板            │
-- │    默认: /clickhouse/tables/{shard}/{table}                 │
-- │                                                              │
-- │  - default_replica_name: 副本名称模板                         │
-- │    默认: {replica}                                          │
-- │                                                              │
-- │  自定义路径示例:                                            │
-- │  /clickhouse/tables/{cluster}/{shard}/{database}/{table}    │
-- │                                                              │
-- │  查询宏变量: SELECT * FROM system.macros;                    │
-- │                                                              │
-- └─────────────────────────────────────────────────────────────┘
...
-- │  - is_readonly: 是否只读模式                                 │
-- │  - is_session_expired: ZooKeeper 会话是否过期                │
-- │  - queue_size: 复制队列大小                                  │
