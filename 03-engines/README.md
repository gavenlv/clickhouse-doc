...
**为什么不能直接存 sum？** 因为 `sum` 已丢失"怎么算的"，无法做"日→月"的二级聚合而不丢精度。状态函数保留了"可继续聚合"的能力，这是 ClickHouse 实时数仓的核心。详见 [04-functions §3](../04-functions/README.md)。
...
- [04-functions](../04-functions/README.md) —— `*State`/`*Merge` 聚合状态函数详解（与本章 §3.4 呼应）
- [16-principle](../16-principle/README.md) —— Part 合并、稀疏索引、向量化执行底层原理
- [11-performance](../11-performance/README.md) —— 引擎选择对查询性能的影响
- [10-date-update](../10-date-update/README.md) —— TTL 与数据生命周期
