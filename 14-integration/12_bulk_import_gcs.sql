...
-- 
-- 本文件涵盖:
--   1. GCS访问权限配置 - HMAC密钥/Service Account/Storage Integration
--   2. 基础导入语法 - gcs()函数/S3兼容语法
--   3. Schema指定方式 - 自动检测vs手动指定
--   4. 性能优化导入 - 并行读取/批量参数
--   5. 高级导入技巧 - Glob模式/过滤/转换
--   6. 错误处理重试 - 超时/重试参数配置
--   7. 最佳实践 - 临时表/物化视图/TTL
-- 
...
-- │                                                                             │
-- │  ┌─────────────────┐          ┌─────────────────┐          ┌─────────────┐ │
-- │  │   GCS Bucket    │          │  ClickHouse     │          │  本地表     │ │
-- │  │                 │          │                 │          │             │ │
-- │  │ ┌─────────────┐ │  HTTP/   │ ┌─────────────┐ │  INSERT  │ ┌─────────┐ │ │
-- │  │ │ file1.parq  │ │───API────►│ │ gcs()函数   │ │─────────►│ │ Table   │ │ │
-- │  │ │ file2.parq  │ │          │ └─────────────┘ │          │ │         │ │ │
-- │  │ │ file3.parq  │ │          │                 │          │ └─────────┘ │ │
-- │  │ └─────────────┘ │          │ ┌─────────────┐ │          └─────────────┘ │
-- │  │                 │          │ │ 并行读取    │ │                           │
...
-- │                               └─────────────────┘                           │
-- │                                                                             │
-- │  数据流: GCS Parquet → gcs()函数 → 并行解析 → 批量写入                         │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ┌─────────────────────────────────────────────────────────────────────────────┐
...
-- │  方式            优点                  缺点                推荐场景          │
-- │  ────────────────────────────────────────────────────────────────────────  │
-- │  gcs()函数       简单直接,自动检测       需配置访问密钥       快速原型开发     │
-- │  S3兼容语法      标准化,兼容性好        需要密钥参数         跨平台迁移       │
-- │  指定Schema      性能最优,类型安全      需手动维护           生产环境        │
-- │  Glob模式        批量导入方便           文件需规范命名        分区数据        │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
