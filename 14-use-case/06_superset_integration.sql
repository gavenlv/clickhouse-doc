...
-- 
-- 本文件涵盖:
--   1. Superset数据源视图 - 优化的BI查询视图
--   2. 指标层级分析视图 - 树形指标下钻支持
--   3. 时间序列分析视图 - 环比/同比计算
--   4. Pivot Table配置 - 透视表维度设置
--   5. 图表配置模板 - 线图/柱图/饼图/热力图
--   6. 仪表板KPI设计 - 关键指标卡片
--   7. 高级分析查询 - 贡献度/异常/对比分析
-- 
-- Superset数据源配置和分析模板
...
-- │  │                                                                   │     │
-- │  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐              │     │
-- │  │  │ superset_   │  │ superset_   │  │ superset_   │              │     │
-- │  │  │ dataset     │  │ metric_     │  │ time_series │              │     │
-- │  │  │ (主数据集)   │  │ hierarchy   │  │ (时序分析)   │              │     │
-- │  │  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘              │     │
-- │  └─────────┼────────────────┼────────────────┼─────────────────────┘     │
-- │            │                │                │                           │
...
-- │  │                                                                   │     │
-- │  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐              │     │
-- │  │  │ Pivot Table │  │   Charts    │  │ Dashboard   │              │     │
-- │  │  │ 透视表分析   │  │ 图表可视化   │  │ 仪表板展示   │              │     │
-- │  │  └─────────────┘  └─────────────┘  └─────────────┘              │     │
-- │  │                                                                   │     │
-- │  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐              │     │
-- │  │  │  Filters    │  │  Metrics    │  │  Calculated │              │     │
-- │  │  │ 筛选器      │  │ 指标配置    │  │ Columns     │              │     │
-- │  │  └─────────────┘  └─────────────┘  └─────────────┘              │     │
-- │  └───────────────────────────────────────────────────────────────────┘     │
-- │                                                                             │
-- │  特点: 视图层优化查询，Superset专注可视化                                       │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ┌─────────────────────────────────────────────────────────────────────────────┐
...
-- │  │  ────────────┼───────────┼───────────┼───────────┼───────────       │   │
-- │  │  2024-01     │  1.2M     │  0.9M     │  1.1M     │  3.2M            │   │
-- │  │  2024-02     │  1.3M     │  0.8M     │  1.0M     │  3.1M            │   │
-- │  │  2024-03     │  1.4M     │  1.0M     │  1.2M     │  3.6M            │   │
-- │  │  ────────────┼───────────┼───────────┼───────────┼───────────       │   │
-- │  │  Total       │  3.9M     │  2.7M     │  3.3M     │  9.9M            │   │
-- │  └─────────────────────────────────────────────────────────────────────┘   │
-- │                                                                             │
-- │  行维度: year_month, metric_name                                            │
-- │  列维度: region_code                                                        │
-- │  值: SUM(prediction_value)                                                  │
-- │  筛选: batch_id=1, metric_level=3                                           │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ┌─────────────────────────────────────────────────────────────────────────────┐
...
-- │  图表类型        适用场景              维度配置              度量            │
-- │  ────────────────────────────────────────────────────────────────────────  │
-- │  时间序列线图    趋势分析              year_month (X轴)      prediction_value│
-- │                                       metric_name (多条线)                  │
-- │                                                                             │
-- │  堆叠柱状图      结构对比              year_quarter (X轴)    prediction_value│
-- │                                       product_category (堆叠)               │
-- │                                                                             │
-- │  饼图            占比分析              customer_segment      prediction_value│
-- │                                                             percentage      │
-- │                                                                             │
-- │  热力图          相关性分析            region_code (Y轴)     prediction_value│
-- │                                       product_category (X轴)                │
-- │                                                                             │
-- │  KPI卡片         核心指标              -                     total_value     │
-- │                                                             avg_value       │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
