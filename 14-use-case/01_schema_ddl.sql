...
-- 
-- 本文件涵盖:
--   1. 维度分离设计原则 - 高基数维度与低基数事实分离
--   2. 二维数组存储优化 - 20指标×60月份压缩为单列
--   3. 分区与排序键策略 - 按批次+年份分区优化查询
--   4. 字典优化JOIN性能 - 内存字典加速维度查找
--   5. 跳数索引设计 - 针对高频过滤字段优化
-- 
-- 场景: 156亿行等效预测数据存储优化
-- 方案: 维度分离 + 数组存储
...
-- │  │  元数据表 (小)   │    │  维度表 (中)     │    │  事实表 (大)     │          │
-- │  │                 │    │                 │    │                 │          │
-- │  │ metric_metadata │    │ trans_dims      │    │ prediction_vals │          │
-- │  │ (20行)          │    │ (13M行/batch)   │    │ (13M行/batch)   │          │
-- │  └────────┬────────┘    └────────┬────────┘    └────────┬────────┘          │
-- │           │                      │                      │                   │
-- │           │     ┌────────────────┴──────────────────────┘                   │
-- │           │     │                                                             │
...
-- │  │  ───────────────┼────────────────────────────────────────── │           │
-- │  │  TXN_000001     │ [[v1,1, v1,2, ...], [v2,1, v2,2, ...], ...]│           │
-- │  │                 │   └─ metric_1 ─┘  └─ metric_2 ─┘           │           │
-- │  │                 │       60个月         60个月                  │           │
-- │  └──────────────────────────────────────────────────────────────┘           │
-- │                                                                             │
-- │  压缩效果: 156亿行 → 1300万行 (99.8%减少)                                      │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
-- ┌─────────────────────────────────────────────────────────────────────────────┐
...
-- │                                                                             │
-- │  ┌──────────────────┐         ┌──────────────────┐                         │
-- │  │  metric_metadata │◄────────│ prediction_values │                         │
-- │  │  (指标元数据)     │  JOIN   │ (预测数据核心表)   │                         │
-- │  └──────────────────┘         └────────┬─────────┘                         │
-- │           ▲                            │                                    │
-- │           │ DICT                       │ JOIN                               │
-- │           │                            ▼                                    │
-- │  ┌──────────────────┐         ┌──────────────────┐                         │
-- │  │  dict_metric_*   │         │ trans_dimensions │                         │
-- │  │  (字典缓存)       │         │ (交易维度表)      │                         │
-- │  └──────────────────┘         └──────────────────┘                         │
-- │                                                                             │
-- │  设计原则: 维度与事实分离，减少数据冗余，提升压缩率                               │
-- │                                                                             │
-- └─────────────────────────────────────────────────────────────────────────────┘
-- 
