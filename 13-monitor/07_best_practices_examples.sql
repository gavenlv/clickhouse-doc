...
--   │   1. 层次化监控                                                        │
--   │   ┌──────────────────────────────────────────────────────────────────┐│
--   │   │ Level 1: 基础资源 (CPU/内存/磁盘/网络)                            ││
--   │   │ Level 2: ClickHouse 进程 (查询/连接/后台任务)                     ││
--   │   │ Level 3: 数据状态 (分区/副本/压缩)                                ││
--   │   │ Level 4: 业务指标 (业务相关监控)                                  ││
--   │   └──────────────────────────────────────────────────────────────────┘│
--   │                                                                        │
--   │   2. 合理告警                                                          │
...
--   │   │ - 热数据: 保留7天, 高精度采集 (1分钟)                             ││
--   │   │ - 温数据: 保留30天, 中精度采集 (5分钟)                            ││
--   │   │ - 冷数据: 保留365天, 低精度采集 (1小时)                           ││
--   │   │ - TTL自动清理: 避免监控数据占用过多空间                           ││
--   │   └──────────────────────────────────────────────────────────────────┘│
--   │                                                                        │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
...
--   
--   ✅ 推荐做法:
--   - 使用 system.asynchronous_metrics (异步采集, 性能影响小)
--   - 限制查询时间范围
--   - 使用物化视图预聚合
--   - 设置查询超时
--   
--   ❌ 避免做法:
--   - 频繁查询 system.query_log
--   - 全表扫描监控表
--   - 复杂的监控查询
--   - 无超时限制的监控
-- 
