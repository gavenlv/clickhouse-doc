...
-- 本文件涵盖:
--   1. 查询优化八法则 - 核心优化原则与技巧
--   2. 减少数据读取 - 列选择/PREWHERE/分区裁剪
--   3. 索引利用优化 - 主键索引/跳数索引/Projection
--   4. 聚合查询优化 - GROUP BY/窗口函数/近似聚合
--   5. JOIN查询优化 - JOIN顺序/类型选择/分布式JOIN
--   6. 排序优化 - ORDER BY与LIMIT配合
--   7. 查询缓存使用 - 缓存配置与命中优化
--   8. 慢查询分析 - EXPLAIN工具与性能诊断
-- 
