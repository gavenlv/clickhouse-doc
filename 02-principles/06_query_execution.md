...
### 2.6 Pipeline 调度器：线程模型（汇编级扩展）

Pipeline 不是简单的"一个算子一个线程"，而是由 **PipelineExecutor** 管理的动态调度系统。理解它的线程模型，才能解释"为什么 max_threads 设置与直觉不同"。

#### 2.6.1 Processor 状态机

每个 Processor 是一个状态机，`execute()` 返回不同状态：

```
enum class Status {
    Ready,          // 还有活要干（继续调用 execute）
    Async,          // 在等待异步 I/O（回调恢复）
    Wait,           // 被 Port 阻塞（等数据或等消费）
    Finished,       // 完成，退出
    PortFull,       // 输出端口满（背压）
    PortChanged     // 端口被重新连接
};
...
#### 2.6.2 两种执行模型：Push vs Pull

| 模型 | 方向 | 谁控制节奏 | ClickHouse 用法 |
|------|------|-----------|----------------|
| **Pull（拉）** | Sink 主动向下游拉取 | 下游（消费端） | 传统 Pipeline（Sink 调上游 execute） |
| **Push（推）** | Source 主动向下游推送 | 上游（生产端） | 异步 I/O Source（Kafka/网络）用 Push |

ClickHouse 从 v20.3 引入 Push 模型后，处理**异步 I/O**（S3、Kafka、远程表）时不再阻塞线程——一个线程可以同时"等待多个 I/O"，大幅提高 I/O 密集查询的吞吐。

#### 2.6.3 线程动态调度

```
PipelineExecutor 调度逻辑（简化）：

while 有未完成任务:
    task = 取出一个 Ready Processor
    thread_pool.submit(task.execute)     # 提交到全局线程池
    if task 返回 Wait:
        task 挂起，等待 Port 事件       # 释放线程，不空转
    if task 返回 Finished:
        task 完成，唤醒下游

关键设计：
- 线程不固定绑定算子：完成一个算子的线程可继续执行下一个算子
- 空闲线程自动回收：无任务时线程休眠，不占 CPU
- max_threads 只是上限：实际并行度由数据量/Part 数动态决定
```
...
```
静态分配（朴素实现）:
  10 个算子 × 8 线程 = 80 线程常驻 → 线程切换开销大

动态调度（ClickHouse）:
  全局线程池（如 16 线程）→ 任意时刻只处理 Ready 任务
  → 线程切换次数少，CPU 利用率高
  → 这就是为什么 max_threads 不必设很大
```

#### 2.6.4 背压（Backpressure）的实现

背压是 Pipeline 稳定性的关键，靠**端口容量**实现：

```
Processor A ──Port──→ Processor B

A.execute() 时:
  如果 B 的输入缓冲未满 → push Block → 返回 Ready
  如果 B 的输入缓冲已满 → 返回 PortFull → A 挂起
  → B 消费掉 Block → 通知 A → A 恢复执行
...
### 3.5 汇编级视角：SIMD 到底怎么工作（R14 汇编级扩展）

前面讲了 SIMD 的"效果"（8 个 Float32 一次算），这一节深入**汇编指令级**，让你真正理解"向量化"在 CPU 上发生了什么。

#### 3.5.1 标量 vs SIMD 的机器码对比

用 `sum(amount)` 中最低层的"累加"为例。假设 8 个 Float32 需要累加：

```
标量版本（无 SIMD）→ 编译器生成 8 条 ADDSS 指令：

  movss  xmm0, [addr+0]      # 加载第 1 个
  addss  xmm0, [addr+4]      # +第 2 个（标量加，每次只加 1 个）
  addss  xmm0, [addr+8]      # +第 3 个
  ...                        # 共 8 条 addss

SIMD 版本（AVX2）→ 编译器生成 1 条 VADDPS + 水平归约：

  vmovups ymm0, [addr+0]     # 一次加载 8 个 Float32（256 位）
  vmovups ymm1, [addr+32]    # 再加载 8 个
  vaddps  ymm0, ymm0, ymm1   # 一条指令完成 8 个并行加法
  ...
  # 最后用 vextractf128 / vhaddps 做水平归约得到总和
```
...
| 指令 | 位宽 | 一次处理 | 用途 |
|------|------|---------|------|
| `SSE2` (ADDSS/PADDD) | 128 位 | 4 × Float32 / 2 × Float64 | 基础向量化 |
| `AVX2` (VADDPS/VFMADD) | 256 位 | 8 × Float32 / 4 × Float64 | 主流（ClickHouse 默认目标） |
| `AVX-512` (VADDPS ZMM) | 512 位 | 16 × Float32 / 8 × Float64 | 服务器 CPU |
| `AVX2` 融合乘加 FMA | 256 位 | 8 × (a×b+c) | 聚合/点积 |

#### 3.5.2 ClickHouse 如何触发 SIMD

ClickHouse 用 C++ 编写，靠**编译器自动向量化** + **手写 intrinsic** 两条路：

```
路径 1：编译器自动向量化（大部分代码）
  for (size_t i = 0; i < size; i++)
      result[i] = a[i] + b[i];
  → GCC/Clang 在 -O3 -mavx2 下自动生成 vaddps
  → 前提：循环无依赖、内存连续（这正是列存的优势！）

路径 2：手写 intrinsic（热点代码）
  #include <immintrin.h>
  __m256 va = _mm256_loadu_ps(&a[i]);   // 加载 8 个 Float32
  __m256 vb = _mm256_loadu_ps(&b[i]);
  __m256 vr = _mm256_add_ps(va, vb);    // 一条指令 8 个加法
  _mm256_storeu_ps(&result[i], vr);     // 存回
  → 用于编译器无法自动向量化的复杂场景
```

#### 3.5.3 为什么列存是 SIMD 的前提（汇编视角）

```
行式存储（ROW）：SELECT sum(amount) FROM t WHERE id < 100

  内存布局: [id0][amount0][name0...][id1][amount1][name1...]...
                                    ↑ 不连续！
  取 amount 需要 stride 跳跃（stride = 行宽）→ 无法 vmovups 连续加载
  → SIMD 失效，退化为标量循环

列式存储（COLUMN）：

  内存布局: [id0][id1][id2]...[amount0][amount1][amount2]...
                                                ↑ 连续！
  vmovups ymm0, [amount_ptr]   ← 一条指令加载 8 个连续 amount
  → SIMD 完美生效
```

**这就是"列存 + 向量化"协同的汇编级证明**：`vmovups` 要求连续内存，只有列存能提供连续的同类型数据。

#### 3.5.4 分支预测与位掩码（Filter 向量化）

WHERE 过滤在向量化下不是"if 分支"，而是**位掩码**：

```
标量: for each row: if (amount > 100) keep(row)   ← 每行一次分支预测

向量化:
  vcmpps  ymm1, ymm0, ymm2, 6    # 比较 8 个 amount > 100，产生掩码
                                  # 6 = CMP_GT_OQ 谓词
  vmaskmovps ymm3, ymm1, ymm0     # 按掩码取走符合条件的 8 个值
  → 没有分支！一条 vcmpps 处理 8 行
  → 分支预测失败率降为 0，这是 PREWHERE 快的重要底层原因
```

#### 3.5.5 验证你的 CPU 支持什么

```sql
-- ClickHouse 内置 CPU 特性查询
SELECT
    cpu_feature,
    available
FROM system.cpu_features
WHERE cpu_feature IN ('avx', 'avx2', 'avx512f', 'sse2', 'fma');

-- 查看 ClickHouse 编译目标
SELECT version() AS version, 'AVX2 默认' AS note;
```
...
- [05-functions/README.md](../05-functions/README.md) —— 聚合状态函数与两阶段聚合
- [08-performance](../08-performance/README.md) —— 查询性能调优实战
