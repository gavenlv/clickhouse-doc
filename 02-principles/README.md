...
关键优化:
  ...
  ③ 聚合分阶段: 先各线程本地聚合 → 再合并 (两阶段聚合, 见 05-functions §3)
...
为什么用两阶段聚合?
  ...
  - sumState/sumMerge 详见 05-functions §3
...
5. **预聚合**：用 AggregatingMergeTree + `*State` 函数（见 05-functions §3）
...
- [04-engines](../04-engines/README.md) —— MergeTree 家族各引擎对比
- [05-functions](../05-functions/README.md) —— *State/*Merge 聚合状态函数（两阶段聚合基石）
- [08-performance](../08-performance/README.md) —— 基于原理的性能优化
- [07-data-mutation](../07-data-mutation/README.md) —— TTL/Mutation/Lightweight Delete 删除机制
