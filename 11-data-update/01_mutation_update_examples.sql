...
-- 
-- 本文件涵盖:
--   1. Mutation 基本语法 - ALTER TABLE ... UPDATE
--   2. Mutation 执行原理 - 异步重写数据
--   3. Mutation 监控 - system.mutations
--   4. Mutation 性能优化 - 分批、分区
--   5. Mutation 最佳实践 - 避免大范围更新
-- 
-- ================================================================================
-- Mutation 工作原理
-- ================================================================================
-- 
--   ┌─────────────────────────────────────────────────────────────────────────┐
--   │                      ClickHouse Mutation 执行流程                       │
--   └─────────────────────────────────────────────────────────────────────────┘
--   
--   用户发起: ALTER TABLE users UPDATE status = 'active' WHERE ...
--                    │
--                    ▼
--   ┌─────────────────────────────────────────────────────────────────────────┐
--   │  1. 创建 Mutation 记录 (system.mutations)                               │
--   │     - mutation_id: mutation_xxx.txt                                     │
--   │     - command: UPDATE ...                                               │
--   │     - is_done: 0 (进行中)                                               │
--   └─────────────────────────────────────────────────────────────────────────┘
--                    │
...
--   磁盘 I/O:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  原 Part (1GB) → Mutation → 新 Part (1GB)                              │
--   │  读取 1GB + 写入 1GB = 2GB 磁盘 I/O                                     │
--   │  即使只更新 1 行数据, 也需要重写整个 Part                                │
--   └────────────────────────────────────────────────────────────────────────┘
--   
--   磁盘空间:
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  Mutation 期间: 原 Part + 新 Part 同时存在                              │
--   │  空间占用翻倍, 直到旧 Part 被清理                                        │
--   └────────────────────────────────────────────────────────────────────────┘
--   
--   CPU/内存:
...
--   ┌────────────────────────────────────────────────────────────────────────┐
--   │  高频更新 → 使用事件日志表 + 查询时合并                                  │
--   │  大范围更新 → 使用分区替换 (REPLACE PARTITION)                          │
--   │  实时状态 → 使用轻量级更新 (ClickHouse 23.8+)                           │
--   └────────────────────────────────────────────────────────────────────────┘
-- 
-- ================================================================================
